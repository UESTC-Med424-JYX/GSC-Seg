...
class VNet_3d_ECL(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False):
        super(VNet_3d_ECL, self).__init__()
    ...
    def forward(self, input):
        features = self.encoder(input)
        cls = self.projection_head(features[-1])
        out_seg, x8_up = self.decoder(features)
        return out_seg, cls  # 4, 16, 112, 112, 80


if __name__ == '__main__':
    net = VNet_3d_ECL(n_channels=1, n_classes=2)

    x = torch.rand(4, 1, 112, 112, 80)
    out, cls = net(x)
