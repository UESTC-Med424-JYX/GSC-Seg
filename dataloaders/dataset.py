...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        image, label, cls = sample['image'], sample['label'], sample['cls']
        if self.with_sdf:
            sdf = sample['sdf']
        ...
        if self.with_sdf:
            ...
            return {'image': image, 'label': label, 'cls': cls}
...
class RandomRotFlip(object):
    ...
    def __call__(self, sample):
        image, label = sample['image'], sample['label']
        image, label, cls = random_rot_flip(image, label)
    ...
    def __call__(self, sample):
        ...
        if 'onehot_label' in sample:
            return {'image': torch.from_numpy(image), 'label': torch.from_numpy(sample['label']).long(),
                    'onehot_label': torch.from_numpy(sample['onehot_label']).long(), 'cls': sample['cls']}
        else:
            return {'image': torch.from_numpy(image), 'label': torch.from_numpy(sample['label']).long(),
                    'cls': sample['cls']}
...
if __name__ == '__main__':
    ...
    x = np.random.randint(low=0, high=100, size=(112, 112, 80))

    print(x.shape)

    y01 = np.flip(np.rot90(x, 0), axis=0)
    y02 = np.flip(np.rot90(x, 1), axis=0)
    y03 = np.flip(np.rot90(x, 2), axis=0)
    y04 = np.flip(np.rot90(x, 3), axis=0)

    y11 = np.flip(np.rot90(x, 0), axis=1)
    y12 = np.flip(np.rot90(x, 1), axis=1)
    y13 = np.flip(np.rot90(x, 2), axis=1)
    y14 = np.flip(np.rot90(x, 3), axis=1)

    print(np.array_equal(x, y01))
    print(np.array_equal(x, y02))
    print(np.array_equal(x, y03))
    print(np.array_equal(x, y04))

    print(np.array_equal(x, y11))
    print(np.array_equal(x, y12))
    print(np.array_equal(x, y13))
    print(np.array_equal(x, y14))
